...
import { lerPlanilhaTurmas } from '../../utils/importarTurmasExcel'
import type { TurmaImportada } from '../../utils/importarTurmasExcel'
...
export function ImportarTurmasModal({ onFechar, onSucesso }: ImportarTurmasModalProps) {
  const { C } = useTheme()
  const inputRef = useRef<HTMLInputElement>(null)

  const [etapa, setEtapa]                 = useState<Etapa>('upload')
  const [arquivo, setArquivo]             = useState<File | null>(null)
  const [turmas, setTurmas]               = useState<TurmaImportada[]>([])
  ...
  const processarArquivo = async (file: File) => {
    ...
    setCarregando(true)
    try {
      const dados = await lerPlanilhaTurmas(file)
      if (dados.length === 0) {
  ...
  // Arquivo fixo em public/modelos/, mesmo padrão do modelo de alunos.
  const baixarTemplate = () => {
    const a = document.createElement('a')
    a.href = '/modelos/Modelo_Upload_Turmas.xlsx'
    a.download = 'Modelo_Upload_Turmas.xlsx'
      ...
      <div style={{ padding:'24px' }}>
        ...
        <div style={{ background:C.surface2, border:`1px solid ${C.border}`, borderRadius:'10px', padding:'14px 16px', marginBottom:'16px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'6px' }}>
            {[
              { col:'A', label:'Nome da turma', obrig:true  },
              { col:'B', label:'Setor',         obrig:false },
              { col:'C', label:'Responsável',   obrig:false },
            ].map(c => (
          ...
          <p style={{ fontSize:'11px', color:C.muted, margin:'8px 0 0' }}>
            * Obrigatório · Turma cujo nome já exista no sistema é rejeitada.
            Ícone, cor e instrutor ficam em branco e podem ser ajustados depois na edição.
          </p>
        </div>
    ...
    return (
      <div style={{ padding:'24px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'10px', marginBottom:'20px' }}>
          {[
            { label:'Total na planilha',   valor:turmas.length,           cor:C.text,    bg:C.surface2  },
            { label:'Prontas p/ importar', valor:turmasValidas.length,    cor:'#10b981', bg:'rgba(16,185,129,0.08)' },
            { label:'Com problemas',       valor:turmasInvalidas.length,  cor:'#ef4444', bg:'rgba(239,68,68,0.08)'  },
          ].map(s => (
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'10px 14px', background:C.surface2, border:`1px solid ${C.border}`, borderRadius:'8px', marginBottom:'16px' }}>
          ...
          <button onClick={() => { setEtapa('upload'); setTurmas([]); setArquivo(null) }}
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ background:C.surface2, position:'sticky', top:0 }}>
                {['#','Nome da turma','Setor','Responsável','Status'].map(h => (
            ...
            <tbody>
              {turmas.map((t, i) => (
                <tr key={i} style={{ borderBottom:`1px solid ${C.border}`, background: t.valido ? 'transparent' : 'rgba(239,68,68,0.04)' }}>
                  <td style={{ padding:'8px 10px', fontSize:'12px', color:C.muted }}>{i+1}</td>
                  <td style={{ padding:'8px 10px', fontSize:'12px', color:C.text, fontWeight:500, maxWidth:'200px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.nome || '—'}</td>
                  <td style={{ padding:'8px 10px', fontSize:'11px', color:C.muted, maxWidth:'160px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.setor || '—'}</td>
                  <td style={{ padding:'8px 10px', fontSize:'11px', color:C.muted, maxWidth:'160px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{t.responsavel || '—'}</td>
                  <td style={{ padding:'8px 10px' }}>
                    {t.valido ? (
                      ...
                      <span title={t.erros.join(' | ')} style={{ fontSize:'10px', fontWeight:700, color:'#ef4444', background:'rgba(239,68,68,0.12)', borderRadius:'4px', padding:'2px 8px', cursor:'help' }}>
                        ✕ {t.erros.length} erro{t.erros.length>1?'s':''}
                      </span>
                    )}
                  </td>
                </tr>
        ...
        {turmasInvalidas.length > 0 && (
          <div style={{ background:'rgba(239,68,68,0.06)', border:'1px solid rgba(239,68,68,0.20)', borderRadius:'8px', marginBottom:'16px', overflow:'hidden' }}>
            ...
            >
              <span>⚠️ Ver detalhes das {turmasInvalidas.length} linha(s) com problema</span>
              {expandirErros ? <ChevronUp size={14} color="#ef4444" /> : <ChevronDown size={14} color="#ef4444" />}
            </button>
            {expandirErros && (
              <div style={{ padding:'0 14px 14px' }}>
                {turmasInvalidas.map((t, i) => (
                  <div key={i} style={{ marginBottom:'8px', padding:'8px', background:'rgba(239,68,68,0.08)', borderRadius:'6px' }}>
                    <p style={{ fontSize:'12px', fontWeight:600, color:'#ef4444', margin:'0 0 4px' }}>
                      Linha {turmas.indexOf(t)+2}: {t.nome || 'Sem nome'}
                    </p>
                    {t.erros.map((e, j) => (
        ...
        {turmasInvalidas.length > 0 && turmasValidas.length > 0 && (
          <div style={{ background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.25)', borderRadius:'8px', padding:'10px 14px', marginBottom:'16px', fontSize:'12px', color:'#f59e0b' }}>
            ⚠️ As {turmasInvalidas.length} linha(s) com problema serão ignoradas.
            Apenas as {turmasValidas.length} válidas serão importadas.
          </div>
        )}

        <div style={{ background:C.surface2, border:`1px solid ${C.border}`, borderRadius:'8px', padding:'10px 14px', marginBottom:'16px', fontSize:'11px', color:C.muted }}>
          Turmas cujo nome já exista no sistema ainda podem ser rejeitadas pelo servidor —
          esta conferência só enxerga a própria planilha.
        </div>

        {turmasValidas.length === 0 && (
          <div style={{ background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.25)', borderRadius:'8px', padding:'10px 14px', marginBottom:'16px', fontSize:'12px', color:'#ef4444' }}>
            ❌ Nenhuma linha válida encontrada. Corrija a planilha e tente novamente.
          </div>
        )}
        ...
        <div style={{ display:'flex', gap:'10px', justifyContent:'flex-end' }}>
          ...
          <button
            onClick={importar}
            disabled={turmasValidas.length === 0}
            style={{ display:'flex', alignItems:'center', gap:'8px', padding:'10px 24px', background:turmasValidas.length>0?C.blue:C.border, border:'none', borderRadius:'8px', fontSize:'13px', fontWeight:700, color:'#fff', cursor:turmasValidas.length>0?'pointer':'not-allowed' }}
          >
            <Upload size={14} />
            Importar {turmasValidas.length} turma{turmasValidas.length!==1?'s':''}
          </button>
        </div>
      </div>
    )
      ...
      <div style={{ padding:'24px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', marginBottom:'20px' }}>
          <div style={{ background:'rgba(16,185,129,0.08)', border:'1px solid rgba(16,185,129,0.25)', borderRadius:'10px', padding:'16px', textAlign:'center' }}>
            ...
            <div style={{ fontSize:'12px', color:C.muted, marginTop:'4px' }}>Importadas com sucesso</div>
          </div>
          <div style={{ background: resultado.erros>0?'rgba(239,68,68,0.08)':C.surface2, border:`1px solid ${resultado.erros>0?'rgba(239,68,68,0.25)':C.border}`, borderRadius:'10px', padding:'16px', textAlign:'center' }}>
            ...
            <div style={{ fontSize:'12px', color:C.muted, marginTop:'4px' }}>Rejeitadas</div>
          </div>
        </div>
          ...
          <div style={{ marginBottom:'16px' }}>
            ...
            <div style={{ maxHeight:'200px', overflowY:'auto', border:`1px solid ${C.border}`, borderRadius:'8px' }}>
              {resultado.detalhes.map((d: any, i: number) => (
                <div key={i} style={{ padding:'8px 12px', borderBottom: i<resultado.detalhes.length-1?`1px solid ${C.border}`:'none' }}>
                  <p style={{ fontSize:'12px', fontWeight:600, color:C.text, margin:0 }}>{d.nome}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {resultado.falhas.length > 0 && (
          <div style={{ background:'rgba(239,68,68,0.06)', border:'1px solid rgba(239,68,68,0.20)', borderRadius:'8px', padding:'12px', marginBottom:'16px' }}>
            <p style={{ fontSize:'12px', fontWeight:700, color:'#ef4444', margin:'0 0 6px' }}>Não importadas:</p>
            {resultado.falhas.map((f: any, i: number) => (
              <p key={i} style={{ fontSize:'11px', color:'#ef4444', margin:'2px 0' }}>
                · {f.nome || '(sem nome)'}: {f.erro}
              </p>
            ))}
          </div>
        )}
        ...
        <div style={{ display:'flex', gap:'10px', justifyContent:'flex-end' }}>
          {resultado.importados < turmasValidas.length && (
