...
/**
 * Conteudo da lista de cursos, SEM layout (nem Sidebar, nem Topbar, nem
 * height:100vh). Segue o padrao dos *Conteudo do projeto (MensagensConteudo,
 * BibliotecaConteudo, CursoDetalheConteudo), que os paineis de admin e
 * instrutor ja reusam. O MeusCursosLista do colaborador virou um involucro
 * fino em volta deste componente, entao ha UMA implementacao para os 3 perfis.
 */
interface MeusCursosConteudoProps {
  onAbrirCurso: (cursoId: string) => void
}

export function MeusCursosConteudo({ onAbrirCurso }: MeusCursosConteudoProps) {
  const { C } = useTheme()
